...
function SaveTemplates() {
    // Save files
    console.log(`${new Date().toLocaleTimeString()} - Saving search-result templates...`);
    fs.writeFile( __dirname + '/search-results.component.php', search_results_template( php_context ), 'utf8', handleErrors );
    fs.writeFile( __dirname + '/search-results.component.html', search_results_template( angular_context ), 'utf8', handleErrors);
}
...
try{
    ...
    args = neodoc.run(`
        ...
        Usage: search-results._template.js [-x|--execute]
        Options:
